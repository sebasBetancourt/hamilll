...
import { PAGE_LINKS } from "@/modules/landing/data/constants";

interface AdeptosWhatsAppCardProps {
  lang: Language;
}
...
export default function AdeptosWhatsAppCard({ lang }: AdeptosWhatsAppCardProps) {
  ...
  return (
    <a
      href={PAGE_LINKS.adeptosWhatsApp}
      target="_blank"
      rel="noopener noreferrer"
      className="link-card link-card--highlight"
    >
      <div className="link-card-left">
        <span className="link-card-icon adeptos-wa-icon">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/logo.png" alt="" width={18} height={18} className="adeptos-wa-logo" />
        </span>
        <div className="link-card-text">
          <span className="link-card-label">{label}</span>
          <span className="link-card-sublabel">{sublabel}</span>
        </div>
      </div>
      <span className="link-card-badge adeptos-wa-badge">
